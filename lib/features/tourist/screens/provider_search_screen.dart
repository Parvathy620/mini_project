...
class _ProviderSearchScreenState extends State<ProviderSearchScreen> {
  ...
  Widget _buildSortOption(String label, String value) {
    ...
    return ListTile(
      ...
      title: Text(label, style: GoogleFonts.inter(color: isSelected ? const Color(0xFF69F0AE) : Colors.white70)),
      trailing: isSelected ? const Icon(Icons.check, color: const Color(0xFF69F0AE)) : null,
    );
  }
        ...
        child: SafeArea(
          child: Column(
            children: [
              ...
              // Load Categories and display chips
              StreamBuilder<List<CategoryModel>>(
                ...
                builder: (context, snapshot) {
                  if (snapshot.hasData) {
                    ...
                    _categoryNames = allCategories.map((c) => c.name).toList();
              ...
              Expanded(
                child: StreamBuilder<List<ServiceProviderModel>>(
                  stream: dataService.getServiceProviders(
                    ...
                    searchQuery: '', // Disable text search for providers, filtering is done via categories
                  ),
                  builder: (context, snapshot) {
                    ...
                    if (_selectedCategories.isNotEmpty) {
                      ...
                      providers = providers.where((p) => p.categoryIds.any((id) => selectedIds.contains(id))).toList();
                    }
                    ...
                    switch (_sortBy) {
                      case 'price_asc':
                        providers.sort((a, b) => a.price.compareTo(b.price));
                        break;
                      case 'price_desc':
                        providers.sort((a, b) => b.price.compareTo(a.price));
                        break;
                      case 'availability':
