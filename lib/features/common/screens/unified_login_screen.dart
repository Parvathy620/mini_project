...
class _UnifiedLoginScreenState extends State<UnifiedLoginScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadLastLogin() async {
    ...
    final lastRoleIndex = prefs.getInt('last_role_index');

    if (mounted && lastEmail != null) {
      setState(() {
        _emailController.text = lastEmail;
        if (lastRoleIndex != null) {
          _selectedRole = UserRole.values[lastRoleIndex];
        }
      });
    }
  }
    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        if (userCredential != null && userCredential.user != null) {
          ...
          await prefs.setInt('last_role_index', _selectedRole.index);
          TextInput.finishAutofillContext(); // Save to Google Smart Lock
  ...
  Future<void> _checkRoleAndRedirect(String uid) async {
    try {
      ...
      if (role == 'provider' && _selectedRole == UserRole.provider) {
        Navigator.of(context).pushReplacement(
          MaterialPageRoute(builder: (context) => const SPDashboardScreen()),
        );
        SharedPreferences.getInstance().then((p) => p.setString('last_route', 'provider'));
        return;
      }

      if (role == 'admin' && _selectedRole == UserRole.tourist) { // Admin logs in as Tourist/Explorer
         Navigator.of(context).pushReplacement(
              MaterialPageRoute(builder: (context) => const DashboardScreen()),
            );
            SharedPreferences.getInstance().then((p) => p.setString('last_route', 'admin'));
            return;
      }

      if (role == 'tourist' && _selectedRole == UserRole.tourist) {
         Navigator.of(context).pushReplacement(
             MaterialPageRoute(builder: (context) => const TouristDashboardScreen()),
          );
          SharedPreferences.getInstance().then((p) => p.setString('last_route', 'tourist'));
          return;
      }
      ...
      if (mounted) {
       ...
       if (mounted) _showGlowSnackBar('Error verifying role: $e', isError: true);
    }
  }
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                const RunwayReveal(
                  delayMs: 200,
                  child: Icon(Icons.public, size: 64, color: const Color(0xFF50C878)),
                ),
                ...
                RunwayReveal(
                  ...
                  child: Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        _buildRoleItem('Explorer', UserRole.tourist), // Unifies Tourist & Admin
                             ...
                             child: Container(
                               ...
                               decoration: BoxDecoration(
                                 ...
                                 gradient: const LinearGradient(
                                   colors: [const Color(0xFF50C878), const Color(0xFF66BB6A)], // Emerald to Soft Green
                                 ),
                                 boxShadow: [
                                   BoxShadow(
  ...
  String _getWelcomeTitle() {
    switch (_selectedRole) {
      ...
      case UserRole.provider: return 'Partner Login';
